...
def get_var_value(var, var_dict):
    '''Get correct value for the variable'''
    if var in var_dict:
        return var_dict[var], var_dict
    else:
        ## All variables (if not b) start with x
        var_dict[var] = 'x' + str(len(var_dict) + 1)
        return var_dict[var], var_dict


def get_drs_from_amr(amr, cur_clauses, arg_dict, level, var_dict, box_num, prev_box_num, cur_arg):
    ...
    for idx, ent in enumerate(split_amr):
        ...
        if level == old_level:
            ...
            if ent == '(':
                ...
                cur_var, var_dict = get_var_value(split_amr[idx-1], var_dict)   #current variable is previous item
                ...
                if split_amr[idx+1] == '(':     #new full AMR, add clauses for current argument here
                    next_var, var_dict = get_var_value(split_amr[idx+2], var_dict)
                    ...
                    if split_amr[idx+1] in var_dict:
                        value = var_dict[split_amr[idx+1]]
            ...
            if ent == '(':
                ...
                if level == old_level and separate_amr:
                    ...
                    cur_clauses,_ , box_num = get_drs_from_amr(" ".join(new_amr), cur_clauses, arg_dict, level, var_dict, box_num + 1, cur_box_num, cur_arg)
...
if __name__ == "__main__":
    ...
    ## Roles that we will always remove from the output since they never occur in a DRS
    remove_roles = ['Wiki', 'Unit', 'Poss', 'Op1', 'Op2']
    ...
    with open(args.drs_file, 'w') as out_f:
        for comments, amr in amrs:
            ...
            ## Remove certain roles that never occur, such as Wiki
            drs = remove_by_type(drs, remove_roles)
