...
def compute_pool(clauses_prod, clauses_gold, prefix1, prefix2, var_count, var_types1, var_types2):
	...
	for i in range(0, len(op_three_vars1)):
		for j in range(0, len(op_three_vars2)):
			...
			candidate_mapping, weight_dict = add_candidate_mapping_three_vars(op_three_vars1, op_three_vars2, prefix1, prefix2, candidate_mapping, weight_dict, var_index1, var_index2, var_index3, edge_index, i, j, var_types1, var_types2, len(op_two_vars2) + len(op_two_vars_abs2),  len(op_two_vars1) + len(op_two_vars_abs1))
	...
	candidate_mapping, weight_dict = map_two_vars_edges(roles_abs1, roles_abs2, prefix1, prefix2, [
														1, 3], candidate_mapping, weight_dict, 0, 2, var_types1, var_types2, len(op_two_vars2) + len(op_two_vars_abs2) + len(op_three_vars2),  len(op_two_vars1) + len(op_two_vars_abs1) + len(op_three_vars1))
	
	# Clause looks like (var1, Role, var2, var3)
	for i in range(0, len(roles1)):
		for j in range(0, len(roles2)):
			var_index1, edge_index, var_index2, var_index3 = 0, 1, 2, 3
			candidate_mapping, weight_dict = add_candidate_mapping_three_vars(roles1, roles2, prefix1, prefix2, candidate_mapping, weight_dict, var_index1, var_index2, var_index3, edge_index, i, j, var_types1, var_types2, len(op_two_vars2) + len(op_two_vars_abs2) + len(roles_abs2) + len(op_three_vars2),  len(op_two_vars1) + len(op_two_vars_abs1) + len(roles_abs1) + len(op_three_vars1))
	...
	candidate_mapping, weight_dict = map_two_vars_edges(concepts1, concepts2, prefix1, prefix2, [
														1, 2], candidate_mapping, weight_dict, 0, 3, var_types1, var_types2, len(op_two_vars2) + len(op_two_vars_abs2) + len(roles_abs2) + len(op_three_vars2) + len(roles2),  len(op_two_vars1) + len(op_two_vars_abs1) + len(roles_abs1) + len(op_three_vars1) + len(roles1))
...
def get_specific_clauses(clause_list, prefix):
	...
	indices_original = [item for sublist in clause_idx_list for item in sublist]
...
def main(arguments):
	...
	#for x,y in zip(clauses_prod_list, clauses_gold_list):
	 #   print len(x), len(y)
